...
const TIP_DISMISS_KEY = "ovLikeTipDismissed";
...
/**
 * D720-like-tip: 하단 액션 바 찜 버튼 — "찜하기" 라벨 + 최초 1회 툴팁
 * "찜하면 캘린더에 자동 등록돼요" (localStorage로 재노출 방지)
 */
export function BookmarkActionButton({ bookmarked, disabled, onClick, accentBorder }: Props) {
  ...
  useEffect(() => {
    try {
      if (!localStorage.getItem(TIP_DISMISS_KEY)) setShowTip(true);
    } catch {
      /* ignore */
    }
  }, []);
