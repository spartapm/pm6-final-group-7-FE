        ...
        return (
          <button
            ...
            className={`flex w-full items-center justify-between px-6 py-4 text-left text-[17px] ${
              selected ? "bg-[#eef0fb] font-bold text-primary" : "font-normal text-[#1c1c27]"
  ...
  return (
    <div className="bg-white">
      <div className="flex items-center gap-1 border-b border-[#eceef2] px-2 py-3">
        ...
        >
          {filters.map((filter) => {
            const selected = Boolean(values[filter.id]);
        ...
        >
          {openFilter.layout === "list" ? (
            <ListFilterPanel
              ...
              value={values[openFilter.id] ?? ""}
              onSelect={(value) => handleSelect(openFilter.id, value)}
            />
          ) : (
            <ChipsFilterPanel
              options={openFilter.options}
